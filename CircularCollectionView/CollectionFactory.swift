...
final class CollectionFactory:NSObject {
    ...
    var delegate:FactoryDataSource?
    ...
    func registerViewModel(vm:Tags)->Tags{
        ...
        guard !existed.isEmpty else {
            vms.append(vm)
            return vm
        }
        guard existed.count == 1 else {return vm}
...
extension CollectionFactory:UICollectionViewDelegate{
